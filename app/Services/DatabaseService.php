...
class DatabaseService
{
    ...
    public function runMaintenance(string $type, ?string $table): array
    {
        ...
        if ($type === 'reset_sequences') {
            DB::statement(<<<'SQL'
                ...
                BEGIN
                    FOR r IN
                        ...
                        JOIN   pg_class     seq ON seq.relname = s.sequence_name AND seq.relkind = 'S'
                        JOIN   pg_depend    d   ON d.objid = seq.oid AND d.deptype = 'a'
                        JOIN   pg_class     t   ON t.oid = d.refobjid
                        JOIN   pg_attribute a   ON a.attrelid = t.oid AND a.attnum = d.refobjsubid
                        WHERE  s.sequence_schema = 'public'
                    LOOP
                        EXECUTE 'SELECT COALESCE(MAX(' || quote_ident(r.col) || '), 0) FROM public.' || quote_ident(r.tbl) INTO v;
                        PERFORM setval('public.' || quote_ident(r.sequence_name), GREATEST(v, 1));
                    END LOOP;
                END $$
                SQL);
            ...
            return [
                'ok'      => true,
                'message' => 'Последовательности синхронизированы',
    ...
    private function runPgDump(string $outputPath): array
    {
        ...
        $cmd = sprintf(
            'PGPASSWORD=%s %s --host=%s --port=%s --username=%s --no-password --format=plain --clean --if-exists %s > %s 2> %s',
