...
class RequestRenderer
{
    ...
    {
        $cacheMinutes = max(0, (int) ($req->cache_time ?? 0));

        if ($cacheMinutes > 0) {
            $pageNum = max(1, (int) $httpRequest->query('page_' . $req->alias, 1));
            $cacheKey = "request:{$req->alias}:p{$pageNum}:x{$currentDocId}";

            return Cache::remember($cacheKey, $cacheMinutes * 60, fn () => $this->doRender($req, $httpRequest, $currentDocId));
        }
    ...
    private function applyFieldCondition(Builder $q, string $field, string $operator, string $value): void
    {
    ...
    private function applySorting(Builder $query, DocsRequest $req): void
    {
    ...
    private function buildGlobalQuery(DocsRequest $req, array $rubricIds, int $currentDocId): Builder
    {
        ...
        $this->applyConditions($query, $req->conditions ?? []);
