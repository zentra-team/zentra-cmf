...
class ApiJsonGenerator
{
    ...
    public function docsPageRubrics(): Collection
    {
        ...
        $fieldsByRubric = RubricField::whereIn('rubric_id', $rubrics->pluck('id'))
            ->where('in_api', true)
            ->orderBy('position')
            ->get(['rubric_id', 'alias', 'title', 'type'])
            ->groupBy('rubric_id');

        return $rubrics->each(function (Rubric $r) use ($fieldsByRubric): void {
            $r->setRelation('apiFields', $fieldsByRubric->get($r->id, collect())->values());
        });
    }
