...
class ModuleManager
{
    ...
    public function downloadFromUrl(string $url, ?string $sha256 = null): array
    {
        try {
            $tempDir = storage_path('app/temp');
            ...
            $tempFile = $tempDir . '/cat_' . md5($url) . '_' . uniqid() . '.zip';

            $response = Http::timeout(120)->get($url);

            if (!$response->ok()) {
                return ['ok' => false, 'message' => 'Не удалось скачать архив модуля (HTTP ' . $response->status() . ')'];
            }
            ...
            if ($sha256 !== null && $sha256 !== '') {
                ...
                if (!hash_equals(strtolower($sha256), strtolower((string) $actual))) {
                    ...
                    return ['ok' => false, 'message' => 'Контрольная сумма архива не совпала — установка отменена'];
                }
            }
    ...
    public function installFromUrl(string $sysName, string $url, ?string $sha256, bool $update): array
    {
        $download = $this->downloadFromUrl($url, $sha256);

        if (!$download['ok']) {
            return $download;
        }

        if (($download['sys_name'] ?? null) !== $sysName) {
            return ['ok' => false, 'message' => 'Имя модуля в архиве не совпадает с каталогом'];
        }

        return $update ? $this->reinstall($sysName) : $this->install($sysName);
    }
    ...
    public function rollbackMigrations(string $sysName): void
    {
        ...
        foreach ($files as $file) {
            try {
                ...
                Log::warning('Возникла ошибка во время отката миграции для модуля', [
                    'module' => $sysName,
                    'file'   => basename($file),
                    'error'  => $e->getMessage(),
                ]);
    ...
    private function extractZip(string $file, string $dest): array
    {
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if ($name !== false && $this->isUnsafeArchivePath($name)) {
                $zip->close();

                return ['ok' => false, 'message' => 'Архив содержит недопустимые пути'];
            }
        }
    ...
    private function extractTarGz(string $file, string $dest): array
    {
        ...
        $entries = [];
        exec(sprintf('tar -tzf %s 2>&1', escapeshellarg($file)), $entries, $listCode);

        if ($listCode !== 0) {
            return ['ok' => false, 'message' => 'Не удалось прочитать tar.gz архив'];
        }

        foreach ($entries as $entry) {
            if ($this->isUnsafeArchivePath($entry)) {
                return ['ok' => false, 'message' => 'Архив содержит недопустимые пути (path traversal)'];
            }
        }
