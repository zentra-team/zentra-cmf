...
class CheckboxListField extends BaseField
{
    ...
    public function renderEdit(mixed $value, array $config): string
    {
        ...
        foreach ($options as $opt) {
            ...
            $sel = in_array($optVal, $selected, true) ? ' checked' : '';
            $html .= '<div class="form-check">'
                   . '<input type="checkbox" id="' . $id . '" class="form-check-input ztr-checkbox-list-item" value="' . e($optVal) . '"' . $sel . '>'
        ...
        if ($template !== null) {
            if (preg_match('/\[items\](.*?)\[\/items\]/s', $template, $m)) {
                ...
                foreach ($values as $i => $v) {
                    $label = $this->resolveLabel((string) $v, $optionsList);
                    $chunk = $itemTpl;
                    $chunk = str_replace('[value]', e($label), $chunk);
                    $chunk = str_replace('[value:raw]', e((string) $v), $chunk);
...
TPL;
    ...
    private function resolveLabel(string $key, array $options): string
    {
        foreach ($options as $opt) {
            [$optKey, $optLabel] = $this->splitOption(trim($opt));
