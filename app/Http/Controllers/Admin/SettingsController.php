...
class SettingsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($canView['general']) {
            $cisTimezones = [
                'Россия' => [
                    'Europe/Kaliningrad', 'Europe/Moscow', 'Europe/Kirov', 'Europe/Volgograd',
    ...
    {
        $url = 'https://api-maps.yandex.ru/2.1/?apikey=' . urlencode($key) . '&lang=ru_RU';
        ...
        $bad = ['apikey is invalid', 'недействительный ключ', '"error":', 'invalid api key'];

        foreach ($bad as $marker) {
            if (stripos($body, $marker) !== false) {
                return response()->json(['ok' => false, 'message' => 'Яндекс отказал: ключ недействителен']);
            }
        }
        ...
        if (strlen($body) < 1000) {
            return response()->json([
                'ok'      => false,
                'message' => 'Яндекс вернул слишком короткий ответ - возможно, ключ недействителен',
            ]);
        }

        return response()->json(['ok' => true, 'message' => 'Ключ работает (SDK доступен)']);
    }
    ...
    {
        foreach (['url_suffix', 'analytics_google', 'analytics_yandex', 'head_code', 'body_code'] as $key) {
            Setting::setValue($key, $request->input($key, ''));
        }
        ...
        app(\App\Services\SitemapGenerator::class)->flush();
        app(\App\Services\RssGenerator::class)->flushAll();
        app(\App\Services\ApiJsonGenerator::class)->flushAll();
    ...
    public function savePublicCache(\App\Http\Requests\Admin\SavePublicCacheSettingsRequest $request, \App\Services\PublicCacheManager $manager): JsonResponse
    {
    ...
    public function flushPublicCache(\App\Services\PublicCacheManager $manager): JsonResponse
    {
        $manager->flushAll();
    ...
    public function sitemapPreview(\App\Services\SitemapGenerator $generator): JsonResponse
    {
        try {
            return response()->json([
                'ok'        => true,
    ...
    public function sitemapFlush(\App\Services\SitemapGenerator $generator): JsonResponse
    {
        $generator->flush();
    ...
    public function testLogNotify(Request $request): JsonResponse
    {
        ...
        if ($telegramEnabled && !empty($token) && !empty($chatId)) {
            ...
            if ($result === false) {
                return response()->json(['ok' => false, 'message' => 'Ошибка отправки в Telegram. Проверьте токен и Chat ID.'], 422);
            }
    ...
    public function composerOutdated(Request $request): JsonResponse
    {
        ...
        try {
            $packages = Cache::remember($cacheKey, 3600, function () {
                ...
                $json = json_decode(trim($outdated['stdout']), true);

                if (!is_array($json)) {
                    $err = trim($outdated['stderr']);

                    throw new \RuntimeException($err !== '' ? $err : 'Неожиданный ответ от composer.');
                }
