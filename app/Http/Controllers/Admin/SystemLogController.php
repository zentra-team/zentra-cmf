...
class SystemLogController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        $users = $canTabAdmin ? User::orderBy('name')->get(['id', 'name']) : collect();
    ...
    {
        if (!Auth::guard('admin')->user()->hasPermission(Permission::LOGS_TAB_FRAMEWORK)) {
            abort(403);
        }
    ...
    private function ensureTabAccess(string $type): void
    {
        ...
        if (!Auth::guard('admin')->user()->hasPermission($perm)) {
            abort(403);
        }
    }
