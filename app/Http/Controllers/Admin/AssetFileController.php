...
class AssetFileController extends Controller
{
    ...
    public function upload(UploadAssetFileRequest $request, string $type): JsonResponse
    {
        if (!in_array($type, ['css', 'js'])) {
    ...
    public function create(CreateAssetFileRequest $request, string $type): JsonResponse
    {
        if (!in_array($type, ['css', 'js'])) {
            return response()->json(['ok' => false, 'message' => 'Неверный тип файла'], 422);
        }
    ...
    public function edit(string $type, string $file): View
    {
        ...
        $canFiles = $user->hasPermission('layouts.files');
    ...
    public function update(UpdateAssetFileRequest $request, string $type, string $file): RedirectResponse
    {
        $path = $this->resolveFile($type, $file);
        $relPath = $this->relPath($type, $path);
    ...
    public function destroy(string $type, string $file): JsonResponse
    {
        $path = $this->resolveFile($type, $file);
        $relPath = $this->relPath($type, $path);
