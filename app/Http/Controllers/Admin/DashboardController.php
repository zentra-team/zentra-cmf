...
class DashboardController extends Controller
{
    ...
    public function stats(Request $request): JsonResponse
    {
        ...
        $tables = ['rubrics' => 'statRubrics', 'requests' => 'statRequests', 'layouts' => 'statLayouts'];
        ...
        try {
            $data['errors_404'] = DB::table('system_logs')->where('type', '404')->count();
        } catch (\Throwable) {
            $data['errors_404'] = 0;
        }

        try {
            $data['errors_sql'] = DB::table('system_logs')->where('type', 'sql_error')->count();
        } catch (\Throwable) {
            $data['errors_sql'] = 0;
        }
    ...
    public function onlineUsers(): JsonResponse
    {
        ...
        return response()->json($users->map(fn ($u) => [
            'id'           => $u->id,
            'name'         => $u->name,
            'last_seen_at' => \Carbon\Carbon::parse($u->last_seen_at)->timestamp,
        ]));
    }
    ...
    {
        try {
            return DB::table('system_logs')->where('type', 'event')->count();
        } catch (\Throwable) {
            return '—';
        }
    }
