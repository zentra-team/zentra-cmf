...
<ul class="nav nav-tabs mb-4" id="moduleTabs">
    ...
    <li class="nav-item">
        <a class="nav-link" data-bs-toggle="tab" href="#tabCatalog">
            <i class="bi bi-shop me-1"></i> Каталог
            @if(count($catalog))
                <span class="badge bg-secondary ms-1">{{ count($catalog) }}</span>
            @endif
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link" data-bs-toggle="tab" href="#tabUpload">
...
<div class="tab-content">
    ...
    <div class="tab-pane fade show active" id="tabInstalled">
        ...
        @if($installed->isEmpty())
            <div class="text-center py-5 text-secondary">
                ...
                <p style="font-size:.875rem">Откройте вкладку «Каталог» чтобы установить первый модуль</p>
            </div>
        @else
                    ...
                    <table class="table table-hover mb-0" style="font-size:.875rem">
                        <thead>
                            <tr>
                                ...
                                <th style="width:150px">Версия</th>
                        ...
                        <tbody>
                            ...
                            <tr id="row_{{ $module->sys_name }}">
                                ...
                                <td class="align-middle" style="white-space:nowrap">
                                    ...
                                    @endif
                                    <span class="catalog-update-badge d-none badge bg-warning text-dark ms-1"
                                          id="cupd_{{ $module->sys_name }}"
                                          style="cursor:pointer;font-size:.7rem"
                                          data-sys-name="{{ $module->sys_name }}"
                                          data-name="{{ $module->name }}"
                                          data-current="{{ $module->version }}">
    ...
    <div class="tab-pane fade" id="tabCatalog">
        ...
        <div class="mb-3">
            <h6 class="mb-1"><i class="bi bi-shop me-1"></i> Каталог Zentra</h6>
            <p class="text-secondary mb-0" style="font-size:.875rem">
                Официальные модули Zentra. Устанавливаются в один клик.
            </p>
        </div>
        ...
        @if(empty($catalog))
            <div class="text-center py-5 text-secondary">
                ...
                <p class="mt-3 mb-0">В каталоге нет новых модулей</p>
                <p style="font-size:.875rem">Похоже, все доступные модули уже установлены</p>
            </div>
        @else
            <div class="row g-3">
                @foreach($catalog as $item)
                <div class="col-12 col-md-6 col-xl-4">
                    <div class="card ztr-card h-100" id="cat_{{ $item['sys_name'] }}">
                        <div class="card-body d-flex flex-column">
                            <div class="d-flex align-items-start justify-content-between mb-2">
                                <span class="fw-semibold" style="color:var(--ztr-purple-400)">{{ $item['name'] ?? $item['sys_name'] }}</span>
                                <span class="badge bg-secondary ms-2">v{{ $item['version'] ?? '—' }}</span>
                            </div>
                            <p class="text-secondary flex-grow-1" style="font-size:.8125rem">
                                {{ $item['description'] ?? '' }}
                            </p>
                            <div class="d-flex align-items-center justify-content-between mt-2">
                                @if(!empty($item['is_paid']))
                                    ...
                                    <span class="text-secondary" style="font-size:.75rem">Покупка — скоро</span>
                                @else
                                    <span class="badge bg-success">Бесплатно</span>
                                    <button class="btn btn-sm btn-primary btn-catalog-install"
                                            data-sys-name="{{ $item['sys_name'] }}"
                                            data-name="{{ $item['name'] ?? $item['sys_name'] }}">
    ...
    <div class="tab-pane fade" id="tabUpload">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <p class="text-secondary mb-0" style="font-size:.875rem">
                Ручная установка: из архива (.zip / .tar.gz) или из папки <code>modules/</code>.
            </p>
        ...
        @if(empty($available))
            <div class="text-center py-5 text-secondary">
                ...
                <p class="mt-3 mb-0">Нет загруженных вручную модулей</p>
...
window.ZentraConfig = {
    routes: {
        ...
        reinstallSource: '{{ route('admin.modules.reinstall-source') }}',
        ...
        catalogInstall: '{{ route('admin.modules.catalog.install') }}',
        catalogUpdate: '{{ route('admin.modules.catalog.update') }}',
        catalogUpdates: '{{ route('admin.modules.catalog.updates') }}'
    }
};
</script>
