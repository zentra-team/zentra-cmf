...
<div class="d-flex gap-2 mb-3 align-items-center flex-wrap">
    ...
    <span id="docStatusBadge" class="badge bg-{{ $document->statusClass() }}">{{ $document->statusLabel() }}</span>
    <span class="text-muted ztr-doc-header-note">
        Рубрика: <strong>{{ $document->rubric?->title ?? '—' }}</strong>
    </span>
    <div class="ms-auto d-flex align-items-center gap-2">
        <span id="docViewBtn">
        @if($document->status === \App\Models\Document::STATUS_ACTIVE && !$document->isScheduled())
        @php
                ...
                <div class="col-12">
                    <label class="form-label">Псевдоним (slug)
                        <i class="bi bi-info-circle text-muted ms-1 ztr-doc-info-icon"
                            ...
                            data-bs-content="Строчные латинские буквы, цифры, дефисы. Слэш <code>/</code> создаёт вложенный путь.<br><br>&#x2728; <strong>Особое значение:</strong><br>Алиас <code>index</code> - главная страница сайта (<code>/</code>).<br>Пустой алиас - главная страница рубрики (<code>/префикс-рубрики</code>).<br><br>Примеры:<br><code>settings</code> → <code>/docs/settings</code><br><code>settings/general</code> → <code>/docs/settings/general</code>"></i>
                    </label>
                    ...
                    <div class="form-text">Строчные латинские буквы, цифры, дефисы. Слэш <code>/</code> создаёт вложенный путь: <code>settings/general</code>. Пустое — главная страница рубрики.</div>
                </div>
                <div class="col-12">
            ...
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <p class="text-muted small mb-2">
                        <i class="bi bi-share me-1"></i><strong>Open Graph</strong> - теги для отображения в соцсетях и мессенджерах. Если не заполнены, тогда будут использованы значения выше.
                    </p>
                </div>
                <div class="col-12">
                    <label class="form-label">OG-заголовок <small class="text-muted">(og:title)</small></label>
                    <input type="text" name="og_title" class="form-control"
                        value="{{ $document->og_title }}"
                        placeholder="{{ $document->meta_title ?: $document->title }}">
                    <div class="form-text">Если пусто, будет использован «Заголовок страницы» или «Название документа».</div>
                </div>
                <div class="col-12">
                    <label class="form-label">OG-описание <small class="text-muted">(og:description)</small></label>
                    <textarea name="og_description" class="form-control" rows="2"
                        placeholder="{{ $document->meta_description ?? 'Краткое описание для соцсетей' }}">{{ $document->og_description }}</textarea>
                    <div class="form-text">Если пусто, тогда будет использовано «Описание страницы».</div>
                </div>
                <div class="col-12">
                    <label class="form-label">OG-изображение <small class="text-muted">(og:image)</small></label>
                    <input type="text" name="og_image" class="form-control"
                        value="{{ $document->og_image }}"
                        placeholder="https://example.com/image.jpg">
                    <div class="form-text">Полный URL картинки. Если не задано, тогда будет использовано глобальное изображение из <strong>Настройки → SEO</strong>. Рекомендуемый размер: 1200×630 пикселей.</div>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <div class="form-text">Если не задано — используется значение из настроек сайта (сейчас: <strong>{{ \App\Models\Setting::getValue('sitemap_default_changefreq', 'weekly') }}</strong>).</div>
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    ...
                    <div class="form-check form-switch mt-2">
                        ...
                        <label class="form-check-label" for="publicCacheDisabled">{{ $document->public_cache_disabled ? 'Не кешировать эту страницу' : 'Кешировать (по настройкам сайта)' }}</label>
                    </div>
                </div>
                <div class="col-md-4">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-5">
                    ...
                    <select name="nav_item_id" class="form-select">
                        ...
                        @foreach($navItemsGrouped as $group)
                        <optgroup label="{{ $group['nav']->title }}">
                            @foreach($group['items'] as $entry)
                            <option value="{{ $entry['item']->id }}" {{ $document->nav_item_id == $entry['item']->id ? 'selected' : '' }}>
                                {{ str_repeat('— ', $entry['depth']) }}{{ $entry['item']->title }}{{ $entry['item']->url ? ' (' . $entry['item']->url . ')' : '' }}
                            </option>
                            @endforeach
                        </optgroup>
                        @endforeach
