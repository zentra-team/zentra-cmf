...
<li class="nav-tree-item" data-id="{{ $item->id }}">
    <div class="nav-item-row {{ $item->is_active ? '' : 'nav-item-inactive' }}">
        ...
        <div class="nav-item-actions ms-auto">
            ...
            @if($canEdit)
            <button type="button"
                class="btn btn-sm {{ $item->is_active ? 'btn-outline-success' : 'btn-outline-secondary' }} btn-status"
    ...
    <ul class="nav-tree-list nav-tree-children"
        ...
        @foreach($item->allChildren as $child)
            @include('admin.navigations._item', ['item' => $child])
        @endforeach
    </ul>
</li>
