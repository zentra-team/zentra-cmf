(function () {
    ...
    document.querySelectorAll('.btn-catalog-install').forEach((btn) => {
        btn.addEventListener('click', async () => {
            const sysName = btn.dataset.sysName;
            setSpinner(btn, true, 'Установка...');

            const { ok, data } = await apiPost(routes.catalogInstall, { sys_name: sysName });
            showToast(data.message, ok && data.ok ? 'success' : 'error');
            setSpinner(btn, false);

            if (ok && data.ok) {
                document.getElementById('cat_' + sysName)?.remove();
                setTimeout(() => location.reload(), 1200);
            }
        });
    });
    ...
    document.querySelectorAll('.btn-reinstall').forEach((btn) => {
        btn.addEventListener('click', () => {
            ...
            document.getElementById('reinstallText').textContent =
                `Переустановить модуль «${btn.dataset.name}» v${btn.dataset.version}? Если модуль из каталога, будет загружена его свежая копия. Данные в таблицах модуля сохранятся.`;
            modalReinstall.show();
        });
    });
    ...
    document.getElementById('btnReinstallConfirm').addEventListener('click', async () => {
        ...
        const { ok, data } = await apiPost(routes.reinstallSource, { sys_name: reinstallTarget });
        modalReinstall.hide();
        showToast(data.message, ok && data.ok ? 'success' : 'error');
        setSpinner(btn, false);
        if (ok && data.ok) {
    ...
    uploadDropZone.addEventListener('click', (e) => {
        if (e.target !== uploadFileInput.labels?.[0]) {
    ...
    async function checkCatalogUpdates() {
        const badges = document.querySelectorAll('.catalog-update-badge');
        if (!badges.length) {
            return;
        }

        try {
            const res = await fetch(routes.catalogUpdates, {
                headers: { Accept: 'application/json' },
            });
            const updates = await res.json();

            Object.entries(updates).forEach(([sysName, info]) => {
                const badge = document.getElementById('cupd_' + sysName);
                if (badge) {
                    badge.textContent = '↑ v' + info.version;
                    badge.classList.remove('d-none');
                    badge.dataset.newVersion = info.version;
    ...
    document.addEventListener('click', (e) => {
        const badge = e.target.closest('.catalog-update-badge');
        if (!badge) {
            return;
        }
        ...
        document.getElementById('updateText').innerHTML =
            `Обновить <strong>«${updateTarget.name}»</strong> с v${updateTarget.current} до v${updateTarget.newVersion}?`;
        document.getElementById('updateChangelog').classList.add('d-none');
    ...
    document.getElementById('btnUpdateConfirm').addEventListener('click', async () => {
        ...
        const { ok, data } =
            updateTarget.source === 'catalog'
                ? await apiPost(routes.catalogUpdate, { sys_name: updateTarget.sysName })
                : await apiPost(routes.update, {
