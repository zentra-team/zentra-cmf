...
function timeAgo(dateStr) {
    ...
    if (weeks === 1)  {return '1 нед назад';}
    if (weeks < 5)    {return weeks + ' нед назад';}
    const months = Math.floor(days / 30);
    if (months === 1) {return '1 мес назад';}
    ...
    ul.innerHTML = logs
        .map((log) => {
            ...
            const iconMap = {
                ...
                edit:   ['bi-pencil', 'update'],
                delete: ['bi-trash', 'delete'],
            };
            const [icon, cls] = iconMap[type] ?? ['bi-gear-fill', 'other'];
        ...
        .map((d) => {
            ...
            <a href="${escapeHtml(editUrl)}" class="ztr-doc-card">
                ...
                <div class="ztr-doc-card-meta">
                    ...
                    <span class="ms-auto">${timeAgo(d.created_at)}</span>
                </div>
            </a>
        </div>`;
        })
