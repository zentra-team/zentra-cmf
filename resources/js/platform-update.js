(function () {
    const cfg = window.ZentraPlatformCfg || window.ZentraConfig || {};
    ...
    const btnReinstall = document.getElementById('btnReinstall');
    ...
    function showState(state) {
        ...
        if (btnReinstall) {
            btnReinstall.classList.toggle('d-none', state !== 'up-to-date');
        }
    }
    ...
    function fillModal() {
        ...
        if (changelog) {changelog.innerHTML = renderChangelog(updateData.changelog || 'Список изменений недоступен.');}
    }
    ...
    modal.addEventListener('show.bs.modal', function () {
        if (!cfg.platformCheckUrl) {fillModal(); return;}

        showState('checking');
        sessionStorage.removeItem('ztr_platform_update');

        fetch(cfg.platformCheckUrl + '?force=1')
    ...
    if (btnReinstall) {
        btnReinstall.addEventListener('click', function () {
            ...
            btnReinstall.innerHTML = '<span class="spinner-border spinner-border-sm me-1"></span>Переустановка…';

            fetch(cfg.platformUpdateUrl + '?force=1', {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': cfg.csrfToken || '',
                    Accept: 'application/json',
                ...
                .then(function (text) {
                    let res;
                    try {res = JSON.parse(text);} catch (e) {res = {ok: false, message: text};}

                    btnReinstall.disabled = false;
                    btnReinstall.innerHTML = '<i class="bi bi-arrow-clockwise me-1"></i>Переустановить текущую версию';

                    if (res.manual) {showManualInstructions(res); return;}

                    if (res.ok) {
                        sessionStorage.removeItem('ztr_platform_update');
                        bootstrap.Modal.getInstance(modal)?.hide();
                        showToast(res.message || 'Переустановка выполнена', 'success');
                        setTimeout(function () {location.reload();}, 2000);
                    } else {
                        showToast(res.message || 'Ошибка переустановки', 'error');
                    }
                })
                .catch(function (e) {
                    btnReinstall.disabled = false;
